...
**Stack (locked at interview):** Expo / React Native + web, one TypeScript
codebase. WatermelonDB as the local engine (SQLite on iOS/Android, LokiJS/
IndexedDB on web) with its pull/push sync protocol. Supabase as the cloud half:
Postgres + RLS, Auth (account required at launch), Storage for item photos,
edge functions for the sync endpoint and metadata-API proxying.
...
REQ-01: Collection/item CRUD on a local-first WatermelonDB data layer that runs on iOS, Android, and web from one codebase
REQ-02: Required Supabase account — sign-up/sign-in gate on all three platforms
REQ-03: Cloud sync — WatermelonDB pull/push protocol against Supabase Postgres with RLS, per-user isolation
