...
- [ ] T1 — Supabase: private `photos` bucket + owner-based RLS on
...
- [ ] T2 — Client wiring: add `photos` Table + `AttachmentTable` to
...
- [ ] T3 — Capture + process: expo-image-picker (camera + multi-select
...
- [ ] T4 — Display: `<ItemPhoto>` component (expo-image; file:// native,
...
- [ ] T5 — Delete: long-press/tap-to-delete with two-tap confirm →
      `queue.deleteFile` + photos row removal; verify remote object gone.
- [ ] T6 — Tests: photos crud round-trip, watchAttachments query shape,
      object-name derivation, errorHandler no-retry decision.
- [ ] T7 — Verify: web capture→upload→Postgres+Storage; second device
