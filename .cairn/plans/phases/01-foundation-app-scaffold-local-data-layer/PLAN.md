...
- [ ] T1 — Scaffold Expo SDK 57 app: TypeScript template, expo-router,
      `expo-dev-client`, web enabled; boots on iOS sim, Android emulator,
      and web (blank shell is fine).
- [ ] T2 — Add PowerSync: `@powersync/react-native` + `@powersync/web`,
...
- [ ] T3 — Define schema in `src/db/schema.ts`: `collections` +
      `items` per CONTEXT.md (custom_fields JSON, money in cents).
- [ ] T4 — Data-access layer: typed CRUD + reactive watch queries
      (`src/db/hooks.ts`) for collections and items; no UI knowledge.
- [ ] T5 — Collections UI: list / create / rename / delete, vertical picked
      at create from a placeholder list (real templates are phase 3).
- [ ] T6 — Items UI: item list within a collection, add/edit/delete with
      common fields (name, notes, acquired date, prices); lists update
      reactively from watch queries.
- [ ] T7 — Test rig: `jest-expo` preset, transformIgnorePatterns for
      PowerSync, unit tests covering schema + CRUD in the data layer.
- [ ] T8 — Cross-platform verify: CRUD flows exercised on iOS, Android,
