...
- [ ] T1 — Adapter interface + registry: `src/metadata/types.ts`
...
- [ ] T2 — Supabase: `metadata` edge function with source routing
...
- [ ] T3 — Direct adapters: Scryfall (2/s throttle, User-Agent), Pokémon
      TCG (keyless), Open Food Facts (bourbon), Funko static dataset
      (ingest JSON, bundle or seed table + text search). (#5)
- [ ] T4 — Proxied adapters: vinyl/Discogs (true barcode lookup), video
...
- [ ] T5 — Scan-to-add UI: scan screen with `CameraView`
...
- [ ] T6 — Tests: registry resolution per template, UPC normalization,
